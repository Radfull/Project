...
class Main(tk.Frame):
    ...
    def init_main(self): # Настройки окошка
        ...
        self.tree = ttk.Treeview(
            self, columns=("ID", "name", "tel", "email", "jokes"), height=45, show="headings"
        )
        ...
        self.tree.column("jokes", width=300, anchor=tk.CENTER)

        self.tree.heading("ID", text="ID")
        self.tree.heading("name", text="ФИО")
        self.tree.heading("tel", text="Телефон")
        self.tree.heading("email", text="E-mail")
        self.tree.heading("jokes", text="Шутка")
    ...
    def records(self, name, tel, email, jokes):
        self.db.insert_data(name, tel, email, jokes)
        self.view_records()
    ...
    def update_records(self, name, tel, email, jokes): # Обновление данных в БД
        self.db.cursor.execute(
            """UPDATE db SET name=?, tel=?, email=?, jokes=? WHERE id=?""",
            (name, tel, email, jokes, self.tree.set(self.tree.selection()[0], "#1")),
        )
        self.db.conn.commit()
        self.view_records()
...
class Child(tk.Toplevel):
    ...
    def init_child(self):
        ...
        self.entry_joke = ttk.Entry(self)
        self.entry_joke.place(x=200, y=140)
        ...
        self.btn_ok.bind(
            ...
            lambda event: self.view.records(
                self.entry_name.get(), self.entry_email.get(), self.entry_tel.get(),  self.entry_joke.get()
            ),
        )
...
class DB:
    ...
    def insert_data(self, name, tel, email, jokes):
        self.cursor.execute(
            """INSERT INTO db(name, tel, email, jokes) VALUES(?, ?, ?, ?)""", (name, tel, email, jokes)
        )
        self.conn.commit()
